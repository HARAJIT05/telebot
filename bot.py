...
from pydub import AudioSegment  # Added import for pydub

TOKEN = '6896380150:AAFVVRSHi6CRUaFShL1MGYtnCIUUrI4_wdM'
OWNER_USER_ID = 6426928410  # Replace with the actual user ID of the bot owner
OPENWEATHERMAP_API_KEY = '8e238375b0ed5d203301c1c6e0ba961f'  # Replace with your OpenWeatherMap API key
YTDL_API_KEY = 'AIzaSyBB0TXBzzv3nZMc7a6LcKBQFUzpdlYFa2U'  # Replace with your YouTube Data API key

bot = telebot.TeleBot(TOKEN)
yt_dl = yt_dlp.YoutubeDL()

# Specify a user agent for Wikipedia API requests
...
def handle_google_search(message: Message, bot, num_results=5):
    try:
        ...
        # Send search results as a message
        results_text = "\n".join(search_results)
...
def handle_weather(message):
    try:
        # Extract city name after the command
...
def handle_wikipedia_search(message):
    try:
        ...
        # Perform Wikipedia search with specified user agent in headers
        ...
        if 'pages' in data['query']:
            ...
            # Remove HTML tags using the remove_html_tags function
...
# Updated YouTube download command
@bot.message_handler(commands=['download'])
def handle_download(message):
    try:
        # Extract the video URL after the command
...
if __name__ == "__main__":
    ...
    print("Bot ID: {}".format(bot.get_me().id))
    print("----------")
